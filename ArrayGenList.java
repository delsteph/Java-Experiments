...
public class ArrayGenList<T> implements GenList<T> {
	 ...
	 *
	 */
	public ArrayGenList(){ //Box array constructor set to a length of 10	
	...
	public void add(T elem) throws NullPointerException {		
		if (elem != null) {   
			  if (isEmpty() == true) {                               //if it is a new list
				   ...
				   for (int i = 0; i < boxes.length;i++ ) {                   
					   if (i == boxIndex) {                     
						   for (int j = boxIndex; j < size(); j++) {       //shifts all elements after index to the right to make room for new element
					   		     boxes[j+1]=boxes[j];    
					   		}
	 ...
	 */
	@Override
	public void add(int index, T elem) throws NullPointerException, IndexOutOfBoundsException { //essentially the same as add method
		if (elem != null) {
	...
	public boolean equals(Object list) {
		...
		if (list instanceof GenList) {                    //checks to see if the object is an instance of GenList
	...
	public T set(int index, T elem) throws NullPointerException, IndexOutOfBoundsException {
        ...
        boxes[index].set(elem);                     //sets the new elem inside box object at that index
